...
//        boolean isLoggedInCustomerMakingRequest; request.getCustomerId().equals(loggedInCustomerId)
    ...
    @Override
    public AppResponse<?> generateAccountStatement(String customerId, String accountNumber, String startDate, String endDate) {
...
//        boolean isLoggedInCustomerMakingRequest; request.getCustomerId().equals(loggedInCustomerId)
        ...
        if (transactions == null || transactions.isEmpty()) {
            ...
            return AppResponse.builder()
                    .responseCode(AccountResponses.SUCCESS.getCode())
                    .responseMessage(AccountResponses.SUCCESS.getMessage() + " :No transactions found for customer within this range")
                    .data(null)
                    .build();
        }

        // TODO transactions to be formatted to pdf then returned to controller

        return AppResponse.builder()
                .responseMessage(AccountResponses.SUCCESS.getCode())
                .responseCode(AccountResponses.SUCCESS.getMessage())
                .data(transactions)
                .build();
    }
